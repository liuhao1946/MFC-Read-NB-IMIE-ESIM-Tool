...
//#define DB_BLE_DATA_BUFFER_SIZE 4
//#define DB_BLE_DATA_BUFFER_POS  3 //DB_BLE_DATA_BUFFER_SIZE-1
...
//注意:严格上该结构并非队列,因为数据出队后依然保留在队列中
//通过app_data_samp_enqueue()把数据循环压到缓存中时，如果
//缓存溢出,则从缓存起始位置开始写入,也就是说会替换掉最先
//进入缓存的数据
...
test_t test[QUEUE_LEN];
test_t ttt[2];
test_t kkk;

1、初始化队列
app_data_samp_init(test, QUEUE_LEN, sizoef(test_t))

2、数据进入队列
kkk.x = 0x01;
kkk.y = 0x02;
kkk.z = 0x03;
app_data_samp_enqueue(&kkk)
...
3、数据出队列
app_data_samp_dequeue(&ttt,2);

//最后入的是最新数据
ttt[0] = {0x04,0x05,0x06}
ttt[1] = {0x01,0x02,0x03}
